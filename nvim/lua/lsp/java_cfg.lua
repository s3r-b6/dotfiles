...
local config = {
	...
	settings = {
		java = {
			home = '/usr/lib/jvm/java-20-jdk',
			eclipse = { downloadSources = true, },
			configuration = {
				updateBuildConfiguration = "interactive",
				runtimes = {
					{ name = "JavaSE-20", path = "/usr/lib/jvm/java-20-jdk", },
			...
			format = {
				enabled = true,
				settings = {
					url = vim.fn.stdpath("config") .. "/lua/fmt/Default.xml",
					profile = "Default",
				},
			},
			edit = {
...
config['on_attach'] = function(client, bufnr)
	vim.api.nvim_set_keymap("n", "<leader>oi", ":lua require('jdtls').organize_imports()<CR>")
